...
def perf(y_true, y_pred, probs, labels):
    ...
    colors = cycle(['cyan', 'steelblue', 'darkgoldenrod', 'firebrick'])
 ...
 #%% Bootsrapping results in test multiclass ###################################
    ...
    for p in metrics:
        ...
        if ((p != roc_auc_score) & (p != metric_aupr_macro) & (p != average_precision_score)): 
    ...
    for p in metrics:
        ...
        if ((p != roc_auc_score) & (p != metric_aupr_micro) & (p != average_precision_score)): 
            y_pred_ = np.argmax(y_pred,axis=1)
            y_true_ = np.argmax(y_true,axis=1)   
        else:
            y_pred_ = y_pred 
